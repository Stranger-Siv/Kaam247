...
function NotificationToast() {
  ...
  return (
    <>
      {/* Mobile: Full screen overlay with scrollable list */}
      <div
        className={`fixed inset-0 z-[1500] bg-slate-950/40 backdrop-blur-[2px] sm:hidden ${fadeClass}`}
        onClick={handleClose}
        style={{
        ...
        >
          ...
          <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-white/10 flex-shrink-0">
            <div className="flex items-center gap-2.5">
              ...
              <h3 className="font-semibold text-lg text-slate-50 tracking-tight">{title}</h3>
            </div>
            <button
              onClick={handleClose}
      ...
      {/* Desktop/Tablet: Scrollable panel (top-right) */}
      <div className={`hidden sm:block fixed top-20 right-4 sm:right-6 z-[1500] ${fadeClass}`}>
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain min-h-0">
            <div className="p-4 space-y-4">
              {notifications.map((n) => (
                <div
                  key={n.id}
                  className="rounded-xl bg-gray-50 dark:bg-gray-700/50 border border-gray-200 dark:border-gray-600 p-4 flex flex-col gap-2.5"
                >
                  <h2 className="text-base font-bold text-gray-900 dark:text-gray-100 leading-tight line-clamp-2">{n.title}</h2>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-gray-200 dark:bg-gray-600 text-gray-800 dark:text-gray-200 text-sm font-medium">
                      {n.category}
                    </span>
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-green-50 dark:bg-green-900/30 text-green-700 dark:text-green-400 text-sm font-bold">
                      ₹{n.budget}
                    </span>
                    {n.distance && (
                  ...
                  {n.location && (
                    <p className="text-sm text-gray-600 dark:text-gray-400 line-clamp-1">{n.location}</p>
                  )}
                  <button
                    onClick={(e) => handleViewTask(e, n.id)}
                    className="w-full px-4 py-2.5 bg-blue-600 dark:bg-blue-500 text-white text-sm font-semibold rounded-lg hover:bg-blue-700 dark:hover:bg-blue-600 transition-colors flex items-center justify-center gap-2"
                  >
