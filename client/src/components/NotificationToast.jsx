...
function NotificationToast() {
  ...
  return (
    <>
      {/* Mobile: full-screen overlay – light mode and dark mode */}
      <div
        className={`fixed inset-0 ${zPanel} bg-gray-500/50 dark:bg-slate-950/50 backdrop-blur-sm sm:hidden ${fadeClass}`}
        onClick={handleClose}
        style={{
      ...
      >
        <div
          className="w-full h-full bg-gradient-to-b from-gray-50 via-white to-gray-50 dark:from-slate-950 dark:via-slate-900 dark:to-slate-950 text-gray-900 dark:text-slate-50 flex flex-col overflow-hidden shadow-2xl border-t border-gray-200 dark:border-transparent"
          onClick={(e) => e.stopPropagation()}
        >
          <div className="flex items-center justify-between px-4 pt-4 pb-3 border-b border-gray-200 dark:border-white/10 flex-shrink-0">
            <div className="flex items-center gap-2.5">
              <div className="w-3 h-3 bg-emerald-500 dark:bg-emerald-400 rounded-full animate-pulse flex-shrink-0" aria-hidden />
              <h3 id="new-task-title-mobile" className="font-semibold text-lg text-gray-900 dark:text-slate-50 tracking-tight">{title}</h3>
            </div>
            <button onClick={handleClose} className="w-10 h-10 flex items-center justify-center text-gray-500 hover:text-gray-900 dark:text-slate-200 dark:hover:text-white rounded-full hover:bg-gray-200 dark:hover:bg-white/10 flex-shrink-0 touch-manipulation" aria-label="Close">
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain px-4 py-4 min-h-0">
            <div className="space-y-4 pb-4">
              {notifications.map((n) => (
                <div key={n.id} className="rounded-xl bg-gray-100 dark:bg-white/5 border border-gray-200 dark:border-white/10 p-4 flex flex-col gap-3 relative overflow-hidden">
                  <button onClick={(e) => handleDismissTask(e, n.id)} className="absolute top-3 right-3 w-8 h-8 flex items-center justify-center text-gray-400 hover:text-gray-700 dark:text-slate-400 dark:hover:text-slate-200 rounded-full hover:bg-gray-300 dark:hover:bg-white/10 touch-manipulation" aria-label="Dismiss this task">
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M6 18L18 6M6 6l12 12" /></svg>
                  </button>
                  <h2 className="text-lg font-bold text-gray-900 dark:text-slate-50 leading-tight line-clamp-2 pr-8">{n.title}</h2>
                  <div className="flex flex-wrap items-center gap-2">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-gray-200 dark:bg-white/10 text-gray-800 dark:text-slate-50 text-sm font-medium">{n.category}</span>
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-emerald-100 dark:bg-emerald-400/15 text-emerald-800 dark:text-emerald-200 text-sm font-bold">₹{n.budget}</span>
                    {n.distance && (
                      <span className="inline-flex items-center gap-1 rounded-lg bg-sky-100 dark:bg-sky-400/15 text-sky-800 dark:text-sky-100 text-sm font-medium px-2.5 py-1">
                  ...
                  {n.location && <p className="text-sm text-gray-600 dark:text-slate-300 line-clamp-1">{n.location}</p>}
                  <button onClick={(e) => handleViewTask(e, n.id)} className="w-full px-4 py-3 bg-blue-600 dark:bg-blue-600 text-white text-base font-semibold rounded-lg hover:bg-blue-700 dark:hover:bg-blue-500 flex items-center justify-center gap-2 touch-manipulation">
                  ...
                  <div className="absolute bottom-0 left-0 right-0 h-1 bg-gray-200 dark:bg-white/10 rounded-b-xl overflow-hidden" aria-hidden>
                    <div className="h-full bg-blue-500 dark:bg-blue-500 rounded-b-xl animate-task-countdown origin-left" style={{ width: '100%' }} />
                  </div>
                </div>
              ))}
            </div>
          </div>
          <div className="px-4 pt-3 pb-4 border-t border-gray-200 dark:border-white/10 flex-shrink-0">
            <p className="text-xs text-gray-500 dark:text-slate-400 text-center">Each task disappears after 15 seconds. Dismiss individually or close all.</p>
          </div>
        </div>
      </div>
