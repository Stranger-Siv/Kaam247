...
import { distanceKm, REACHED_LOCATION_RADIUS_KM } from '../utils/geoDistance'
...
function TaskDetail() {
  ...
  const [isCheckingReached, setIsCheckingReached] = useState(false)
  const [reachedLocationError, setReachedLocationError] = useState(null)
  ...
  // Reached location (worker): verify within 200m of task, then unlock Start task
  const handleReachedLocation = () => {
    if (isCheckingReached || !task) return
    ...
    setIsCheckingReached(true)
    setReachedLocationError(null)
    if (!navigator.geolocation) {
      setReachedLocationError('Location is not supported. You can still continue.')
      setHasReachedLocation(true)
      setIsCheckingReached(false)
      return
    }
    navigator.geolocation.getCurrentPosition(
      (position) => {
        const taskLng = coords[0]
        const taskLat = coords[1]
        const workerLat = position.coords.latitude
        const workerLng = position.coords.longitude
        const km = distanceKm(taskLat, taskLng, workerLat, workerLng)
        if (km <= REACHED_LOCATION_RADIUS_KM) {
          setHasReachedLocation(true)
          setReachedLocationError(null)
        } else {
          setReachedLocationError("You're not at the task location yet. Please go to the task address and try again.")
        }
        setIsCheckingReached(false)
      },
      () => {
        setReachedLocationError('Could not get your location. Enable location access and try again.')
        setIsCheckingReached(false)
      },
  ...
  const renderPrimaryAction = () => {
    ...
    if (isWorkerForTask || (!isPosterForTask && userMode === 'worker')) {
      if (currentStatus === 'SEARCHING' || currentStatus === 'OPEN' || task.status === 'open') {
        ...
        // Worker must confirm "Reached location" (within 200m) before Start task is shown; chat is available as soon as accepted
        if (!hasReachedLocation) {
          return (
            <div className="space-y-3">
              ...
              <div className="w-full px-6 py-4 bg-amber-50 dark:bg-amber-900/30 text-amber-800 dark:text-amber-200 text-base font-medium rounded-lg text-center border border-amber-200 dark:border-amber-700">
                Go to the task location, then tap below when you've arrived.
              </div>
              {reachedLocationError && (
                <div className="p-3 bg-red-50 dark:bg-red-900/30 border border-red-200 dark:border-red-800 rounded-lg">
                  <p className="text-sm text-red-700 dark:text-red-400">{reachedLocationError}</p>
                </div>
              )}
              <button
                onClick={handleReachedLocation}
                disabled={isCheckingReached}
                className="w-full px-6 py-4 bg-emerald-600 dark:bg-emerald-500 text-white text-lg font-semibold rounded-lg hover:bg-emerald-700 dark:hover:bg-emerald-600 transition-colors shadow-md hover:shadow-lg disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center gap-2"
              >
                {isCheckingReached ? (
                  <>
                    <svg className="animate-spin h-5 w-5 text-white" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
